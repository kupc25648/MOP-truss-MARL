...
class gen_model:
    ...
    def render_load(self,load='Y',factor=0.01,name='./name.png',target=None):
        ...
        for i in range(len(self.model.elements)):
            if self.model.elements[i].section_no !=0:
                ...
                elementplot.plot(x='X',y='Y',kind='line',color ='red',alpha=0.5,grid=True, ax=n,legend=False,  linewidth= thick*5)
...
test = gen_model(num_x,num_y,span_x,span_y,tar_y,dmin,loadx,loady,truss_type,support_case,topo_code)


test.render_load(load='Y',factor=0.01,name='./name.png',target=tar_xy)
#print(test.model.U_full)
