...
def simple_cull(Allpoint,optional=False):
    ...
    if optional==True:
        ...
        print('________________________')
    '''
    all_pass_maxx = sorted(all_pass_maxx[-max([1,min(10,len(all_pass_maxx))]):], key=lambda x: x[1])
    '''
    if optional==True:
        print(all_pass_maxx)
        print('________________________________________________')
    ...
    all_pass_maxy = sorted(all_pass_maxy[-max([1,min(10,len(all_pass_maxy))]):], key=lambda x: x[0])
    ...
    for i in range(min([len(all_pass_maxx),3])):
        try:
            is_front_and_edges.append(all_pass_maxx[-i-1])
        except:
            pass
    for i in range(min([len(all_pass_maxy),3])):
        try:
...
def simple_cull_final(Allpoint,optional=False):
    inputPoints = []#Allpoint.copy()
    all_points = []
    '''
    '''
