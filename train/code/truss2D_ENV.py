...
class Game_research04:
    ...
    def _game_modify(self,set_node, set_element, nC_e, actions):
        ...
        #print('----------------------------------------------------')
        #print(np.dot(nC_e,actions[1]).shape)
        #print(len(self.gen_model.model.elements))
        modify_element = np.dot(nC_e,actions[1])
        for i in range(len(self.gen_model.model.elements)):
            '''
            print(modify_element[i].shape)
            print(np.argmax(modify_element[i]))
            s_node   = self.gen_model.model.elements[i].nodes[0].name
            e_node   = self.gen_model.model.elements[i].nodes[1].name

            pred_val_1 = np.dot(actions[1][s_node-1][:50].T,actions[1][e_node-1][:50])/len(actions[1][0])
            pred_val_2 = np.dot(actions[1][s_node-1][50:100].T,actions[1][e_node-1][50:100])/len(actions[1][0])
            pred_val_3 = np.dot(actions[1][s_node-1][100:150].T,actions[1][e_node-1][100:150])/len(actions[1][0])
            pred_val_4 = np.dot(actions[1][s_node-1][150:200].T,actions[1][e_node-1][150:200])/len(actions[1][0])
            pred_val_5 = np.dot(actions[1][s_node-1][200:].T,actions[1][e_node-1][200:])/len(actions[1][0])

            pred_vals = np.array([pred_val_1, pred_val_2, pred_val_3, pred_val_4, pred_val_5])
            '''
            pred_vals = modify_element[i]
            # rule not to reduce
            '''
            if self.gen_model.model.elements[i].section_no > np.argmax(pred_vals):
                if self.gen_model.model.elements[i].prop_yeield <= threshold_stress:
                    # change
                    self.gen_model.model.elements[i].section_no = np.argmax(pred_vals)
