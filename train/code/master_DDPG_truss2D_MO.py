...
from utils import dominates, simple_cull, CoverQuery, union_rectangles_fastest
import ast
import gc
...
def gen_train_choice(num_x,num_y,num_choice):
    choice = [ [] for i in range(num_choice) ]
    for i in range(num_choice):
        span_x    = [ random.randint(1,5) for i in range(num_x-1) ]
        span_y    = [random.randint(15,20)]
        tar_y     = [ round((span_y[0] - 10) * random.random(),2) for i in range(num_x) ]
        max_y_val = span_y[0]
        min_y_val = 0
        choice[i] = [ span_x, span_y, tar_y, max_y_val, min_y_val]
...
#gen_num_y  = 2
#num_choice = 4
#trainChoice = gen_train_choice(gen_num_x,gen_num_y,num_choice)
#testChoice = [[[3, 5, 3, 5, 3], [5], [11.77, 9.31, 9.92, 11.31, 11.57, 11.95], 12.5, 0]]
...
span_x = [4,3,5,3,5]
span_y = [7]
tar_y  = [1.0,3.0,3.0,2.0,1.5,1.0]
dmin = sum(span_x)*0.01
...
loady = -25000
support_case = 4
test = gen_model(num_x,num_y,span_x,span_y,tar_y,dmin,truss_src,loadx,loady,support_case)
'''



trainChoice = [
                [[4.0,3.0,5.0,3.0,5.0],[5],[1.0,1.5,2.0,2.0,1.5,1.0],5,0],
                [[4.0,3.0,5.0,3.0,5.0],[5],[1.0,3.0,3.0,2.0,1.5,1.0],5,0],
                [[4.0,3.0,5.0,3.0,5.0],[5],[1.0,1.5,2.0,3.0,3.0,1.0],5,0],
                [[4.0,3.0,5.0,3.0,5.0],[5],[1.0,3.0,2.0,2.0,3.0,1.0],5,0],
                [[4.0,3.0,5.0,3.0,5.0],[5],[3.0,2.0,1.0,1.0,2.0,3.0],5,0]
                ]

testChoice = [[[5.0,5.0,5.0,5.0,5.0,5.0,5.0],[8],[4.0,3.0,2.5,2.0,2.0,2.5,3.0,4.0],8,0]]
print('TRAIN')
print(trainChoice)
print('TEST')
print(testChoice)
while counter < num_episodes:
    ...
    # ==========================================
    measure_num = 10
    if (counter+0)%measure_num != 0:

        game_choice = [random.choice(trainChoice)]
        num_x = len(game_choice[0][0])+1
        num_y = len(game_choice[0][1])+1
        dmin = 0.2
        tar_y = game_choice[0][2]#[ max([game_choice[0][2][i]*(5*random.random()),dmin]) for i in range(len(game_choice[0][2])) ] # add some randomness + guarntee dmin
        support_case = 1#[random.choice([1,2,3,4])]
        truss_type = random.choice(['roof','bridge'])
        #game_choice = testChoice
        '''
        ...
        '''
        if env_model == None:
            env_model = gen_model(num_x,num_y,game_choice[0][0],game_choice[0][1],game_choice[0][2],dmin,gen_load_x,gen_load_y,truss_type,gen_topo_code,support_case)
        else:
            env_model.re_value(num_x,num_y,game_choice[0][0],game_choice[0][1],game_choice[0][2],dmin,gen_load_x,gen_load_y,truss_type,gen_topo_code,support_case)
        ...
        '''
        if env_model == None:
            env_model = gen_model(num_x,num_y,game_choice[0][0],game_choice[0][1],tar_y,dmin,gen_load_x,gen_load_y,truss_type,gen_topo_code,support_case)
        else:
            env_model.re_value(num_x,num_y,game_choice[0][0],game_choice[0][1],tar_y,dmin,gen_load_x,gen_load_y,truss_type,gen_topo_code,support_case)
        if game == None:
            game = Game_research04(env_end_step,env_model,env_num_agents)
        else:
            game.re_game(env_end_step,env_model,env_num_agents)
        ...
        super_step += run(game,train_period,savedata=0)
        game_reward = [0,0,0,0] # reward_g, reward_t, global_r
        ...
        truss_type = 'roof'
        if env_model == None:
            env_model = gen_model(num_x,num_y,testChoice[0][0],testChoice[0][1],testChoice[0][2],dmin,gen_load_x,gen_load_y_eval,truss_type,gen_topo_code,support_case)
        else:
            env_model.re_value(num_x,num_y,testChoice[0][0],testChoice[0][1],testChoice[0][2],dmin,gen_load_x,gen_load_y_eval,truss_type,gen_topo_code,support_case)
    ...
    if counter%save_iter == 0:
        name_of_Actor_pickle = "Actor_pickle"
        name_of_Critic_pickle = "Critic_pickle"
        ...
        for i in range(num_agents):
            name_of_Actor_pickle = "Agent{}_Actor_pickle".format(i+1)
            name_of_Critic_pickle = "Agent{}_Critic_pickle".format(i+1)
            ...
            Actor_picklename = os.path.join(pickle_path, name_of_Actor_pickle)
            Critic_picklename = os.path.join(pickle_path, name_of_Critic_pickle)
            reinforcement_learning.agents[i].actor_model.save_weights(Actor_picklename)
            reinforcement_learning.agents[i].critic_model.save_weights(Critic_picklename)
